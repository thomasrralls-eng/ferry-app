...
const FIX_SUGGESTIONS = {
  ...
  "param-value-too-long": {
    ...
    steps: [
      ...
      "gd ferry auto-detects your GA4 version — if detection is wrong, you can override it in settings",
    ],
  },
  "ga4-version-detected": {
    summary: "GA4 property version was detected from client-side signals",
    steps: [
      "gd ferry adjusts parameter value limits, custom dimension limits, and other thresholds based on your GA4 version",
...
function deriveInsights(events, network, findings) {
  ...
  if (events.length === 0 && network.length === 0) {
    insights.push({
      ...
      detail: "Ferry didn't capture any dataLayer pushes or GA4 network hits. Check that the site has GA4 or GTM installed.",
    });
    return insights;
  }
  ...
  if (findings.length === 0 && totalEvents > 5) {
    insights.push({
      ...
      detail: "Ferry checked all events against GA4 best practices and found no issues.",
    });
  }
